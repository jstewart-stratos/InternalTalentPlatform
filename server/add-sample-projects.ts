...
async function addSampleProjects() {
  console.log("Adding sample projects to existing database...");

  // Get all employees to assign as project owners
  const allEmployees = await db.select().from(employees);
  
  if (allEmployees.length === 0) {
    console.log("No employees found. Cannot add projects.");
    return;
  }
  ...
  if (existingProjects.length > 0) {
    console.log("Projects already exist. Clearing existing projects...");
